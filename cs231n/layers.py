...
def affine_forward(x, w, b):
    ...
    N = s[0]
    d1_k = s[1:]
    num_elem = np.prod(d1_k)
    X = x.reshape(N, num_elem)
...
def affine_backward(dout, cache):
    ...
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx.reshape(N, *d1_k), dw, db
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x, m, v, d, s, f, t, y, gamma, beta)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, m, v, d, s, f, t, y, gamma, beta = cache 
    ...
    # dy/dt = 1
    # dy/dbeta = 1
    # dL/dt = dy/dt * dL/dy = 1.0 * dout
    # dL/dbeta = dy/dbeta * dL/dy = 1.0 * dout
    ...
    # dt/dgamma = f
    # dt/df = gamma
    # dL/gamma = dt/dgamma * dL/dt
    dgamma = np.sum(f * dt, axis=0)
    df = gamma * dt
    
    # backprop f = d * s^-1
    # df / dd = s^-1
    # df / ds = -d * s^-2 = -d / s^2
    # dL / dd = df/dd * dL/df = 1 / s  * df
    # dL / ds  = df / ds * dL/df = -d/s^2 * df
    dd = np.divide(1.0, s) * df
    ds = np.divide(-d, np.square(s)) * df
    
    # backprop s = sqrt(v) 
    # we have dL/ds
    # we want dL/dv = ds/dv * dL/ds
    # ds/dv = 1 / 2 * sqrt(v)
    dv = np.divide(1.0, 2.0 * np.sqrt(v)) * ds
    ...
    # backprop v = var(x) = 1/D * Sum(1,D) (x - m)^2 
    # dx/dv * dL/dv = dL/dx
    dx += 2.0 / D * (x - m) * dv
